 ...
 * @author    
 */
class Lof_Coinslider_Adminhtml_BannerController extends Mage_Adminhtml_Controller_Action {
    protected function _initAction() {
        $this->loadLayout()
                ->_setActiveMenu('coinslider');
    ...
    public function indexAction() {
        ...
        $this->_initAction();
        $this->_addContent($this->getLayout()->createBlock('lof_coinslider/adminhtml_banner'));
        $this->renderLayout();
    ...
    public function addAction() {
        ...
        $this->_addContent($this->getLayout()->createBlock('lof_coinslider/adminhtml_banner_add'))
                ->_addLeft($this->getLayout()->createBlock('lof_coinslider/adminhtml_banner_add_tabs'));
    ...
    public function editAction() {		
        ...
        $_model  = Mage::getModel('lof_coinslider/banner')->load($bannerId);
        ...
        if ($_model->getId()) {
            ...
            $this->getLayout()->getBlock('head')->setCanLoadExtJs(true);
            $this->_addContent($this->getLayout()->createBlock('lof_coinslider/adminhtml_banner_edit'))
                    ->_addLeft($this->getLayout()->createBlock('lof_coinslider/adminhtml_banner_edit_tabs'));
			
            $this->renderLayout();
        } else {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('lof_coinslider')->__('The banner does not exist.'));
            $this->_redirect('*/*/');
        }
    }
        ...
        if ($data = $this->getRequest()->getPost()) {
			...
			if(isset($_FILES['file']['name']) && $_FILES['file']['name'] != '') {					
					try {	
						...
						$path = Mage::getBaseDir('media') . '/coinslider/upload/';
					...
					$data['file'] = 'coinslider/upload/' . $_FILES['file']['name'];
				}else{
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('lof_coinslider')->__('Banner was successfully saved'));
    ...
    public function deleteAction() {
        if( $this->getRequest()->getParam('id') > 0 ) {
            try {
                $model = Mage::getModel('lof_coinslider/banner');
    ...
    public function massStatusAction() {
        ...
        if(!is_array($IDList)) {
            ...
            try {
                foreach ($IDList as $itemId) {
                    $_model = Mage::getSingleton('lof_coinslider/banner')
    ...
    public function imageAction() {
        ...
        try {
            ...
            $result = $uploader->save(
                    Mage::getSingleton('lof_coinslider/config')->getBaseMediaPath()
            );

            $result['url'] = Mage::getSingleton('lof_coinslider/config')->getMediaUrl($result['file']);
            $result['cookie'] = array(
                    'name'     => session_name(),
