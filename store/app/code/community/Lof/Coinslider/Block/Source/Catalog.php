 ...
 */
if(!class_exists("Lof_Coinslider_Block_List")){
	require_once dirname(__FILE__).DIRECTORY_SEPARATOR."List.php";
}
class Lof_Coinslider_Block_Source_Catalog extends Lof_Coinslider_Block_List 
{
	/**
	...
	function _toHtml() 
	{
		...
		if( $this->getConfig('source') == 'catalog' )
		{
	...
	private function __renderSlideShowProducts() 
	{
		...
		$tmp = $listall->getItems() ;
		$this->assign( 'items', $listall->getItems() );
		$theme = ($this->getConfig('theme')!="") ? $this->getConfig('theme') : "default";
		$this->_config['template'] = 'lof/coinslider/'.$theme.'/list_products.phtml';	
	}
	...
	function getListBestBuyProducts($fieldorder = 'ordered_qty', $order = 'desc', $product_ids = '', $perPage = NULL, $currentPage = 1) {
		...
		*/
        ...
        if($this->_config['catsid']){
            // get array product_id
            $arr_productids = $this->getProductByCategory($product_ids);    
            $products = Mage::getResourceModel ( 'catalog/product_collection' )
                        ->setStoreId ( $storeId )
                        ->addAttributeToSelect ( '*' )
                        ->addStoreFilter ( $storeId )
                        ->setOrder ( $fieldorder, $order )
                        ->addAttributeToFilter('entity_id', array('in' => $arr_productids))
                        ;
        }else{
            $products = Mage::getResourceModel ( 'catalog/product_collection' )
            			->setStoreId ( $storeId )
    ...
    function getProductByCategory($product_ids = array()){
        $return = array(); 
        $pids = array();
        
        if(!empty($product_ids)){
        	$product_ids = !is_array($product_ids)?explode(",", $product_ids):$product_ids;
        	$products = Mage::getResourceModel ( 'catalog/product_collection' )->addIdFilter($product_ids);
        }else{
        	$products = Mage::getResourceModel ( 'catalog/product_collection' );
        }
