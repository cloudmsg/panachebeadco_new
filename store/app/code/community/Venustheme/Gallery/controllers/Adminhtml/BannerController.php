...
class Venustheme_Gallery_Adminhtml_BannerController extends Mage_Adminhtml_Controller_Action {
    ...
    public function indexAction() {
        ...
        $this->_initAction();
        $this->_addContent($this->getLayout()->createBlock('venustheme_gallery/adminhtml_banner') );
        $this->renderLayout();
    ...
    public function addAction() {
        ...
        $_model  = Mage::getModel('venustheme_gallery/banner');
        ...
        $this->_addContent($this->getLayout()->createBlock('venustheme_gallery/adminhtml_banner_add'))
                ->_addLeft($this->getLayout()->createBlock('venustheme_gallery/adminhtml_banner_add_tabs'));
   ...
   public function editAction() {		
        ...
        $_model  = Mage::getModel('venustheme_gallery/banner')->load($bannerId);
        ...
        if ($_model->getId()) {
            ...
            $this->getLayout()->getBlock('head')->setCanLoadExtJs(true);
            $this->_addContent($this->getLayout()->createBlock('venustheme_gallery/adminhtml_banner_edit'))
                    ->_addLeft($this->getLayout()->createBlock('venustheme_gallery/adminhtml_banner_edit_tabs'));
			
            $this->renderLayout();
        } else {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('venustheme_gallery')->__('The record does not exist.'));
            $this->_redirect('*/*/');
        }
    }
    ...
    public function saveAction() {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('venustheme_gallery')->__('Record was successfully saved'));
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('venustheme_gallery')->__('Unable to find record to save'));
        $this->_redirect('*/*/');
    }
    ...
    public function deleteAction() {
        if( $this->getRequest()->getParam('id') > 0 ) {
            try {
                $model = Mage::getModel('venustheme_gallery/banner');
    ...
    public function massStatusAction() {
        ...
        if(!is_array($IDList)) {
            ...
            try {
                foreach ($IDList as $itemId) {
                    $_model = Mage::getSingleton('venustheme_gallery/banner')
    ...
    public function imageAction() {
        $result = array();
        try {
            $uploader = new Venustheme_Gallery_Media_Uploader('image');
            $uploader->setAllowedExtensions(array('jpg','jpeg','gif','png'));
            $uploader->setAllowRenameFiles(true);
            $uploader->setFilesDispersion(true);
            $result = $uploader->save(
                    Mage::getSingleton('venustheme_gallery/config')->getBaseMediaPath()
            );

            $result['url'] = Mage::getSingleton('venustheme_gallery/config')->getMediaUrl($result['file']);
            $result['cookie'] = array(
                    'name'     => session_name(),
