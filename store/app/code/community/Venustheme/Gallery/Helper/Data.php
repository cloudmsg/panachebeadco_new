<?php
class Venustheme_Gallery_Helper_Data extends Mage_Core_Helper_Abstract {
	...
	function get( $attributes = NULL)
	{
		...
		{	    	
			$tags = array('venustheme_gallery', 'effect_setting', 'file_source_setting','image_source_setting', 'main_gallery');
			foreach($tags as $tag){
				if(Mage::getStoreConfig("venustheme_gallery/$tag/$var")!=""){
					$data[$var] =  Mage::getStoreConfig("venustheme_gallery/$tag/$var");
				}
			}
        ...
        return Mage::getSingleton('venustheme_gallery/config')->getBaseMediaUrl() . $url;
    }
