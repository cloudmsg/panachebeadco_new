...
-------------------------------------------------------------------------*/
class Venustheme_Blog_Adminhtml_CategoryController extends Mage_Adminhtml_Controller_Action {
    protected function _initAction() {
        $this->loadLayout()
                ->_setActiveMenu('venustheme_blog/category');
    ...
    public function indexAction() {
        ...
        $this->_initAction();
        $this->_addContent($this->getLayout()->createBlock('venustheme_blog/adminhtml_category') );
        $this->renderLayout();
	...
	public function editAction(){
        ...
        $_model  = Mage::getModel('venustheme_blog/category')->load( $id );
		...
		$this->loadLayout();
	    $this->_setActiveMenu('venustheme_blog/category');
        ...
        $this->_addContent($this->getLayout()->createBlock('venustheme_blog/adminhtml_category_edit'))
                ->_addLeft($this->getLayout()->createBlock('venustheme_blog/adminhtml_category_edit_tabs'));
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {	    
			$model = Mage::getModel('venustheme_blog/category');
			if($data['identifier'] == '' || !isset($data['identifier']))
			$data['identifier'] = Mage::helper('venustheme_blog')->formatUrlKey($data['title']);
			...
			if(isset($_FILES['file']['name']) && $_FILES['file']['name'] != '') {					
				...
				foreach( $sizes as $key => $size ){
					$c = Mage::getStoreConfig( 'venustheme_blog/category_setting/'.$key );
					$tmp = explode( "x", $c );
					if( count($tmp) > 0 && (int)$tmp[0] ){
						Mage::helper('venustheme_blog')->resizeImage( $data['file'], $size, (int)$tmp[0], (int)$tmp[1] );
					}
				}		
			...
			try {
				$model->save();
				$resroute = Mage::getStoreConfig('venustheme_blog/general_setting/route');
				$extension = ".html";
				//Save to Url Rewite
				...
				Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('venustheme_blog')->__('Category was successfully saved'));
		...
		Mage::getSingleton('adminhtml/session')->addError(Mage::helper('venustheme_blog')->__('Unable to find cat to save'));
		$this->_redirect('*/*/');
    }
	...
	public function imageAction() {
        $result = array();
        try {
            $uploader = new Venustheme_Blog_Media_Uploader('image');
            $uploader->setAllowedExtensions(array('jpg','jpeg','gif','png'));
            $uploader->setAllowRenameFiles(true);
            $uploader->setFilesDispersion(true);
            $result = $uploader->save(
                    Mage::getSingleton('venustheme_blog/config')->getBaseMediaPath()
            );

            $result['url'] = Mage::getSingleton('venustheme_blog/config')->getMediaUrl($result['file']);
            $result['cookie'] = array(
                    'name'     => session_name(),
	 ...
	 public function deleteAction() {
		...
		if( $this->getRequest()->getParam('id') > 0 ) {
			try {
				...
				Mage::getModel('core/url_rewrite')->loadByIdPath('venustheme_blog/category/'.$model->getId())->delete();
			...
			foreach( $collection as $post ){
				if( $post->getFile() ){
					...
					foreach( $sizes as $key => $size ){
						...
						$tmp = explode( "x", $c );
						if( count($tmp) > 0 && (int)$tmp[0] ){
							Mage::helper('venustheme_blog')->resizeImage( $post->getFile(), $size, (int)$tmp[0], (int)$tmp[1] );
						}
					}	
				}
			}
        ...
        if(!is_array($IDList)) {
            ...
            try {
                foreach ($IDList as $itemId) {
                    $_model = Mage::getSingleton('venustheme_blog/category')
	...
	public function massDeleteAction() {
        ...
        if(!is_array($IDList)) {
            ...
            try {
                foreach ($IDList as $itemId) {
                    $_model = Mage::getModel('venustheme_blog/category')
