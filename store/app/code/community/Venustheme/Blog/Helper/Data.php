...
-------------------------------------------------------------------------*/
class Venustheme_Blog_Helper_Data extends Mage_Core_Helper_Abstract {
	...
	function get( $attributes = NULL)
	{
		...
		{	    	
			$tags = array('venustheme_blog', 'effect_setting', 'file_source_setting','image_source_setting', 'main_blog');
			foreach($tags as $tag){
				if(Mage::getStoreConfig("venustheme_blog/$tag/$var")!=""){
					$data[$var] =  Mage::getStoreConfig("venustheme_blog/$tag/$var");
				}
			}
        ...
        return Mage::getSingleton('venustheme_blog/config')->getBaseMediaUrl() . $url;
    }
	public function resizeImage( $image,$type="l", $width, $height ){
		...
		return 'resized/'."{$type}/".str_replace(DS,"/",$image);
	}
    /**
	...
	public function getTagUrl($tag){
		return Mage::getBaseUrl().Mage::getStoreConfig('venustheme_blog/general_setting/route')."/tag/".$tag.".html";
	}
