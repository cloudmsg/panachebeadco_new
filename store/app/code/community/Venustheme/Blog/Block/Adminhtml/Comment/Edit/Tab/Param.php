...
class Venustheme_Blog_Block_Adminhtml_Comment_Edit_Tab_Param extends Mage_Adminhtml_Block_Widget_Form
{
    protected function _prepareForm()
    {
        ...
        $fieldset = $form->addFieldset('comment_params', array('legend'=>Mage::helper('venustheme_blog')->__('Parameter')));
		
		$fieldset->addField('template', 'select', array(
			'label'     => Mage::helper('venustheme_blog')->__('Template'),
			'name'      => 'param[template]',
			'values'    => array( 0=> $this->__("No"), 1=> $this->__("Yes") )
		));
		
		$fieldset->addField('show_childrent', 'select', array(
			'label'     => Mage::helper('venustheme_blog')->__('Show Childrent'),
